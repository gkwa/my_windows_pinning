...
pinto10 /unpintb "${env:SYSTEMDRIVE}\Program*\Windows Media Player\wmplayer.exe"
...
foreach($h in $list) {
    ...
    # duplicates
    pinto10 /unpintb $h.get_item("ShortcutFilePath")
}
...
foreach($h in $list) {
    ...
    pinto10 /pintb $h.get_item("ShortcutFilePath")
}
